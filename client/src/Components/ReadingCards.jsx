    ...
    <a href={ book.Link } target='_blank' rel='noopener noreferrer'>
      <div className='relative group h-96 w-72'>
        ...
        <div className='relative h-96 w-72 cursor-pointer rounded-xl items-center justify-center overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30'>
          ...
          <div className='absolute inset-0 flex opacity-0 translate-y-[60%] flex-col items-center justify-center px-9 text-white text-center transition-all duration-500 group-hover:opacity-100 group-hover:translate-y-0'>
            <h2 className='text-2xl font-bold'>{ book.Title }</h2>
            <p className='mb-3 text-lg italic opacity-0 transition-opacity duration-300 group-hover:opacity-100'>{ book.Author }</p>
            <div className={`text-sm italic ${statusColor} rounded-xl px-2 py-1 opacity-0 transition-opacity duration-300 group-hover:opacity-100`}>{ book.ExclusiveShelf }</div>
          </div>
        </div>
      </div>
    </a>
